...
**영체(아군) 소환은 실험 기능** — `ally` 명령으로 스폰 개체를 영체 팀(47)에 넣으면 황금빛으로 빛나며 적과 싸우지만, **플레이어도 물어뜯는 경우가 있어** 기본 설정에서는 `disabled` 에 넣어 두었습니다. (Hexinton 의 Recruit 도 같은 방식이라 같은 한계.) 쓰려면 `config.json` 의 `disabled` 에서 빼고 직접 확인하세요. `dismiss` 로 전부 해제.
